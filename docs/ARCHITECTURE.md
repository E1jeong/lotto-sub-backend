...
Android client completes Google Play purchase
  ...
  -> backend returns the receipt verification result
...
- The main-server issuance-sync call happens only after the DB transaction commits, so its failure never affects entitlement state or the receipt response.
