...
async function postLoopback(
  ...
  try {
    ...
    if (!res.ok || data?.status !== '8200') {
      console.error(failureMessage, data?.status ?? res.status);
    }
  } catch (e) {
...
export async function requestExpectNumberIssuance(
  ...
  await postLoopback(
    REISSUE_URL,
    { email, phone },
    '[main-server] 1077 예상번호 발급 동기화 실패:',
    '[main-server] 1077 호출 오류:',
  );
}
...
export async function requestInitialExpectNumberIssuance(params: {
  ...
  await postLoopback(
    ...
    '[main-server] 1022 무료 예상번호 초기 발급 실패:',
    '[main-server] 1022 호출 오류:',
  );
}
